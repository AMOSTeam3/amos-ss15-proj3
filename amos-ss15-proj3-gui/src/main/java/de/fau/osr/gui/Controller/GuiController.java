...
import de.fau.osr.core.vcs.bl.Tracker;
...
//
    ...
    void requirementsFromFile() {
        ...
        Supplier<Collection<? extends DataElement>> fetching = () -> {
            ...
            try{
                return i_Collection_Model.getRequirementsFromFile((Collection)files);
            } catch (IOException e) {
        ...
        for (int i = 0; i <= MAX_RETRIES; i++) {
            ...
            try {
                guiModelTrial = reInitModel(null, null, repoFile,
                        Pattern.compile(i_Collection_Model
                                .getCurrentRequirementPatternString()));
    ...
    private Collection_Model_Impl reInitModel(VcsClient vcs, DataSource ds,
        ...
        return new Collection_Model_Impl(new Tracker(vcs, ds, repoFile));
    }
